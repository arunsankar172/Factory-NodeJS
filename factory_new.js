    ...
    con.connect(function (err) {
        ...
        con.query('SELECT (SELECT SUM(kw*0.000972222) AS "usage" from energy WHERE DAY(time) = DAY(CURRENT_DATE())) as "today" ,(select "0") "thisweek", (select "0") as "thismonth";', function (err, result, fields) {
            ...
            if (result.length > 0) {
                ...
                console.log("App Res: " + JSON.stringify(response));
...
app.get('/update1_data/', (req, res) => {
    ...
    kw = data.kw
    const kwParsed = parseFloat((parseFloat(data.kw)/1000).toFixed(3));
    console.log("Incomming Kw: " + kwParsed);
    console.log("Incomming Data: " + JSON.stringify(data));

    if (parseFloat(kwParsed) < 1000 && parseFloat(data.vp1) < 1000 && parseFloat(data.vp2) < 1000 && parseFloat(data.vp3) < 1000) {
        ...
        con.connect(function (err) {
            ...
            con.query("INSERT INTO energy (kw, vp1, vp2 ,vp3, vry, vyb, vbr ,il1 ,il2 ,il3, pf) VALUES (?,?,?,?,?,?,?,?,?,?,?) ;", [kwParsed, data.vp1, data.vp2, data.vp3, data.vry,
            data.vyb, data.vbr, data.il1, data.il2, data.il3, data.pf], function (err, result) {
                if (err) {
                    if (err.code === 'ER_DUP_ENTRY') {
                        ...
                        console.error('Invalid field error:', err.message);
                    } else {
                        console.error('Database error:', err.message);
                    }
                    return;
                }
                console.error('Updated Data in Energy DB');
...
app.get('/iframe1', (req, res) => {
    ...
    fs.readFile('barchart.html', 'utf-8', function (err, data) {
        con.connect(function (err) {
            if (err) throw err;
            con.query("SELECT SUM(kw*0.000972222) AS y, Date(time) AS x FROM energy GROUP BY DATE(time) ORDER BY Date(x) DESC LIMIT 7;", function (err, result, fields) {
                if (err) throw err;
                if (result.length > 0) {
                    response = result;
                    console.log(response);
